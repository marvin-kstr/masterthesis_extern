...
M_ref = 2048 #temporal discretization
N_ref = 100 #galerkin discretization  ## Note that this is recommended by Kloeden and Jentzen!
J = 100 #spacial discretization
T = 1 #length temporal interval
S = 1 #length spacial interval
kappas = [2,4,8,16,32,64,128,256]
K =50
...
for k in tqdm(range(K)):
	...
	for i in kappas:
		sum_montecarlo_zw = 0
		kappa=i
		approx = kappa_nonlinear_exp_euler(M_ref,N_ref,J,T,S,kappa,rand_lst_all)
		sum_dim = 0
...
result_x = [int(M_ref/i) for i in kappas]
result_y = [results[i] for i in range(len(results))]

line_x = np.linspace(8,1024,1000)
